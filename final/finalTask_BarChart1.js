class BarChart1 {
    ...
    update() {
        ...
        const data_map = d3.rollup( self.data, v => v.length, d => d.time );
    ...
    render() {
        ...
        self.chart.selectAll(".bar")
            ...
            .on('click', function(ev,d) {
                const is_active1 = filter1.includes(d.key);
                if ( is_active1 ) {
                    filter1 = filter1.filter( f => f !== d.key );
                }
                else {
                    filter1.push( d.key );
                }
                Filter();
                d3.select(this).classed('active', !is_active1); //バーの縁取り(クリックの可視化)
            });
